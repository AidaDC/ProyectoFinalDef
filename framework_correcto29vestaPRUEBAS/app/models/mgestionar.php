...
where c.Usuarios_id_usuario = ?";
 ...
 function gestionarusuarios()
  ...
  try
    {
    $sql = "SELECT id_libro,imagen,description,ISBN,price,idioma,valoration,stock,edi.nombre,n_paginas,edition,lib.nombre,aut.nombre as 'nombre autor',aut.apellidos
...
<p >Precio: ' . $fila['price'] . '</p>');
 ...
 FROM libros lib INNER JOIN  editorial edi on lib.editorial = edi.id_editorial 
 inner join libros_autor la on lib.id_libro = la.Libros_id_libro 
 inner join autor aut on la.Autor_id_autor = aut.id_autor where lib.id_libro != ? LIMIT 3";
      $query = $this->db->prepare($sql);
      $query->bindParam(1, $idlib);
      $query->execute();
      if ($query->rowCount() > 0)
...
<div id="der"><div id="descrip"><p>Nombre:  ' . utf8_encode($fila['nombre']) . '</p>
  <p>Autor:' . utf8_encode($fila['nombre autor']) . ' ' . utf8_encode($fila['nombre autor']) . '</p>
  <p >Precio:  ' . $fila['price'] . '</p>
